...
- [x] **Native ACP mode**: `autohand --mode acp` for direct editor integration
- [x] In-process ACP agent (no external adapter subprocess needed)
- [x] Full ACP protocol: ndJSON over stdio, session management, tool streaming
- [x] **Zed Editor integration**: seamless coding assistant with permission UI
- [x] Session modes: Interactive, Full Access, Unrestricted, Auto Mode, Restricted, Dry Run
- [x] Tool call streaming with kind classification (read, edit, search, execute, think)
- [x] Permission bridge: mode-aware approval flow (auto-approve, auto-deny, interactive prompt)
- [x] Session lifecycle: create, resume, fork, list sessions
- [x] Config options: thinking level, auto-commit, context compaction
- [x] 24 slash commands exposed to editor UI
