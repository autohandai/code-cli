...
describe("Encryption Utilities", () => {
  ...
  describe("encryptConfig", () => {
    it("encrypts apiKey fields", () => {
      ...
      expect((encrypted.openrouter as Record<string, unknown>).baseUrl).toBe(
        "https://openrouter.ai/api/v1",
      );
      expect(
        isEncrypted(
          (encrypted.openrouter as Record<string, unknown>).apiKey as string,
        ),
      ).toBe(true);
    });
    ...
    it("encrypts nested API keys", () => {
      ...
      const encrypted = encryptConfig(config, testToken);
      const providers = encrypted.providers as Record<
        string,
