...
describe("SetupWizard — Reasoning Effort", () => {
  ...
  it("should NOT prompt reasoning effort for non-OpenAI providers", async () => {
    setupNonOpenAICloud("openrouter", "your-modelcard-id-here");
  ...
  it("should show reasoning effort in review summary", async () => {
    ...
    const logCalls = (console.log as any).mock.calls
      .map((c: any[]) => c[0])
      .filter(Boolean);
    const hasReasoningLog = logCalls.some(
      (msg: string) =>
  ...
  it("should print the auth error message when chatgpt sign-in fails", async () => {
    mockAuthenticateOpenAIChatGPT.mockRejectedValueOnce(
      new Error("device auth forbidden"),
    );
    ...
    await expect(wizard.run({ skipWelcome: true })).rejects.toThrow(
      "device auth forbidden",
    );
    ...
    expect(
      logCalls.some(
        (msg: string) =>
          typeof msg === "string" &&
          msg.includes("providers.openaiAuth.failed"),
      ),
    ).toBe(true);
  });
});
