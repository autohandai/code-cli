...
function fillConversation(
  ...
  for (let i = 0; i < count; i++) {
    cm.addMessage({
      role: "user",
      content: `Request ${i}: ${"x".repeat(contentLength)}`,
    });
    cm.addMessage({
      role: "assistant",
      content: `Response ${i}: ${"y".repeat(contentLength)}`,
    });
  }
}
...
describe("LLM-Powered Context Summarization", () => {
  ...
  it("should include accomplished work details from LLM summary", async () => {
    ...
    const messages: LLMMessage[] = [
      ...
      },
      {
        role: "tool",
        name: "write_file",
        content: "Created src/auth/jwt.ts",
        tool_call_id: "tc1",
      },
      {
        role: "tool",
        name: "write_file",
        content: "Modified src/auth/index.ts",
        tool_call_id: "tc2",
      },
    ];
  ...
  it("should persist key facts to memory during summarization", async () => {
    ...
    const messages: LLMMessage[] = [
      ...
      {
        role: "assistant",
        content: "I chose PostgreSQL over MySQL because...",
      },
    ];
  ...
  it("summarizeMessages alias should work the same as summarizeMessagesStatic", () => {
    const messages: LLMMessage[] = [
      { role: "user", content: "Fix the bug" },
      {
        role: "tool",
        name: "read_file",
        content: "file content of src/index.ts",
      },
    ];
...
describe("Tiered context management with LLM summarization", () => {
  ...
  it("should use LLM summarization in Tier 2 when context crosses 80%", async () => {
    const llm = createMockLLM(
      "Summary: user asked to refactor auth. Files modified: auth.ts, index.ts.",
    );
