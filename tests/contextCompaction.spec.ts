...
describe("Context Compaction", () => {
  describe("ContextManager Integration", () => {
    ...
    beforeEach(() => {
      ...
      contextManager = new ContextManager({
        model: "your-modelcard-id-here", // 200k context window
        conversationManager,
      });
    });
    ...
    it("does not emit no-op summaries when only the active turn is large", async () => {
      ...
      expect(
        conversationManager.history().filter((msg) => msg.role === "system"),
      ).toHaveLength(1);
    });
    ...
    it("removes the selected low-priority messages during critical compaction", async () => {
      ...
      const initialAssistantCount = conversationManager
        .history()
      ...
      expect(
        result.messages.filter((msg) => msg.role === "assistant").length,
      ).toBeLessThan(initialAssistantCount);
    });
  });
  ...
  describe("Retry Logic Pattern Fix", () => {
    ...
    it('should correctly identify "context is too long" error', () => {
      const message =
        "the request was malformed. context is too long".toLowerCase();
