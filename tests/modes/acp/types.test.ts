...
describe("resolveToolDisplayName()", () => {
  ...
  it("returns formatted MCP name for mcp__ prefixed tools", () => {
    expect(resolveToolDisplayName("mcp__my_server__my_tool")).toBe(
      "MCP: my_server/my_tool",
    );
    expect(resolveToolDisplayName("mcp__context7__query-docs")).toBe(
  ...
  it("returns Title Case for unknown snake_case tools", () => {
    expect(resolveToolDisplayName("some_unknown_tool")).toBe(
      "Some Unknown Tool",
    );
...
describe("parseAvailableModels()", () => {
  ...
  it("places the configured model first when it exists", () => {
    const config = makeConfig({
      openrouter: { apiKey: "sk-test", model: "your-modelcard-id-here" },
    });
    const models = parseAvailableModels(config);

    expect(models[0]).toBe("your-modelcard-id-here");
  });
  ...
  it("does not duplicate the configured model if it is in the popular list", () => {
    ...
    const models = parseAvailableModels(config);
    const occurrences = models.filter((m) => m === "your-modelcard-id-here");
