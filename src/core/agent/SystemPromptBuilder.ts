...
export class SystemPromptBuilder {
  ...
  async build(): Promise<string> {
    ...
    const parts: string[] = [
      ...
      '1. Read ALL relevant files before planning. Use `fff_find` first for filename/path discovery, `find_grep` for content discovery, then `read_file` once you know the exact file or region to inspect.',
      ...
      '   - After access is granted, continue with dedicated file tools (read_file, fff_find, find_grep, etc.).',
      '',
      '#### Search Optimization',
      ...
      '- Use `find_grep` for content/code discovery. It auto-detects regex, falls back to fuzzy on zero matches when native FFF is available, classifies definitions, and includes git annotations.',
      '- Use `fff_find` first when you need file discovery by filename, extension, or path pattern.',
      '- Use `find_grep` as the default code discovery tool for content, symbols, imports, and regex lookup.',
      '- `find_grep` features: smart-case, definition classification, context lines, git status annotations.',
      '- Use `find_grep` and `fff_find` for all new searches.',
      ...
      '- Prefer dedicated file tools (`fff_find`, `find_grep`, `read_file`, `git_status`, `git_diff`) over `run_command` whenever they can accomplish the task.',
      ...
      '- The legacy tools `search`, `search_with_context`, and `semantic_search` are compatibility aliases. Prefer `find_grep` for new tool calls.',
      '- Examples:',
      '  - File discovery: `fff_find(query="**/*.test.ts")` or `fff_find(query="auth controller")`',
      '  - Content search: `find_grep(query="UserController")` or `find_grep(query="async function.*login")`',
      '',
      ...
      '',
      'Use `recall_memory` for ranked retrieval by content, tags, and recency.',
      'Use `inspect_memory(operation="outline")` when memory is too large for a flat list, then zoom into returned summary node IDs without changing the snapshot.',
      'The derived summaries are disposable caches: `inspect_memory(operation="forget")` invalidates them, while `operation="rebuild"` restores JSON projections from canonical memory event history.',
      'Use `delete_memory` only for an explicitly obsolete memory; deletion remains recorded in canonical memory event history.',
      '',
